...
export default function ReviewFormPage() {
  ...
  const [form, setForm] = useState({
    ...
    // 授業情報
    courseName: '',
    teacherNames: [''] as string[], // 1人目必須、追加可
  ...
  const removeTeacher = (index: number) => {
    setForm((prev) => {
      if (index === 0) return prev; // 1人目は消せない
  ...
  const isFormValid = useMemo(() => {
    ...
    // 教員名：1人以上（空白は除外した後に判定）
    if (normalizedTeacherNames.length < 1) return false;
    ...
    // コメント長
    if (form.comment.trim().length < MIN_COMMENT_LENGTH) return false;
    ...
    // 年度はデフォルトで入っている想定（念のため範囲）
    if (form.academicYear < 1990 || form.academicYear > 2100) return false;

    return true;
  }, [form, normalizedTeacherNames, creditsValue]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        teacher_names: normalizedTeacherNames,
    ...
    <main className="flex min-h-screen justify-center px-3 py-4 sm:px-4">
      <div className="w-full max-w-xl space-y-4 rounded-2xl bg-white/80 p-4 shadow-soft backdrop-blur-sm">
        ...
        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <SectionCard title="授業情報" subtitle="科目名と教員名を入力してください（教員は追加できます）">
            <div className="grid gap-4">
              <div className="field-wrapper">
                ...
                {form.teacherNames.map((name, idx) => (
                  <div key={idx} className="grid gap-2">
                    ...
                    <input
                      id={`teacher-${idx}`}
                      className="control"
                      placeholder={idx === 0 ? '例：山田太郎' : '例：共同担当の先生（任意）'}
                      value={name}
